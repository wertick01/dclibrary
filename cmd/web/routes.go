...
func CreateRoutes(userHandler *handlers.UsersHandler, carsHandler *handlers.BooksHandler) *mux.Router {
	...
	r.HandleFunc("/api/users", db.UsersStorage.GetUsersList).Methods("GET")
	r.HandleFunc("/api/users/{id:[0-9]+}").Methods("GET")
	r.HandleFunc("/api/users/self").Methods("GET")
	r.HandleFunc("/api/users").Methods("POST")
	r.HandleFunc("/api/users/{id:[0-9]+}").Methods("PUT")
	r.HandleFunc("/api/users/{id:[0-9]+}").Methods("DELETE")
