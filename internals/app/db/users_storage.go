...
func (m *UsersStorage) GetUsersList() ([]*models.FinalUser, error) {
	...
	var snippets []*models.FinalUser
	//var rol m

	for rows.Next() {
		s := &models.FinalUser{}
		err = rows.Scan(&s.User.UserId, &s.User.Name, &s.User.Phone, &s.User.Mail, &s.User.RoleId)
		if err != nil {
			return nil, err
		}
		rol := m.DB.QueryRow(sdmd, s.User.RoleId)
		err = rol.Scan(&s.Role.RoleId, &s.Role.Role)
...
func (m *UsersStorage) GetUserById(id int64) (*models.FinalUser, error) {
	...
	err := row.Scan(&s.User.UserId, &s.User.Name, &s.User.Phone, &s.User.Mail, &s.User.RoleId)
	if err != nil {
