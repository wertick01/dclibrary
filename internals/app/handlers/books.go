...
func (handler *BooksHandler) Find(w http.ResponseWriter, r *http.Request) {
	...
	user, err := handler.processor.FindBook(id)
	if err != nil {
		WrapError(w, err)
		return
	}
...
func (handler *BooksHandler) FindByAuthor(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r) //переменные, обьявленные в ресурсах попадают в Vars и могут быть адресованы
	if vars["id"] == "" {
		WrapError(w, errors.New("missing id"))
		return
...
func (handler *BooksHandler) Change(w http.ResponseWriter, r *http.Request) {
	...
	user, err := handler.processor.UpdateBook(id)
	if err != nil {
		WrapError(w, err)
		return
	}
...
func (handler *BooksHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	deleteduser, err := handler.processor.DeleteBook(id)
	if err != nil {
		WrapError(w, err)
		return
	}
